...
export function getOpenClawClaudeMemExtensionDirectory(): string {
  return path.join(getOpenClawExtensionsDirectory(), 'hummem');
}
...
function registerPluginInOpenClawConfig(
  ...
  config.plugins.slots.memory = 'hummem';

  if (!config.plugins.entries['hummem']) {
    config.plugins.entries['hummem'] = {
      enabled: true,
      config: {
        workerPort,
        project,
    ...
    config.plugins.entries['hummem'].enabled = true;
    if (!config.plugins.entries['hummem'].config) {
      config.plugins.entries['hummem'].config = {};
    }
    const existingPluginConfig = config.plugins.entries['hummem'].config;
...
function unregisterPluginFromOpenClawConfig(): void {
  ...
  if (config.plugins?.entries?.['hummem']) {
    delete config.plugins.entries['hummem'];
  }

  if (config.plugins?.slots?.memory === 'hummem') {
...
export function checkOpenClawStatus(): number {
  console.log('\nhummem OpenClaw Integration Status\n');
  ...
  if (existsSync(configFilePath)) {
    const config = readOpenClawConfig();
    const isRegistered = config.plugins?.entries?.['hummem'] !== undefined;
    const isEnabled = config.plugins?.entries?.['hummem']?.enabled === true;
    const isMemorySlot = config.plugins?.slots?.memory === 'hummem';
    ...
    if (isRegistered) {
      const pluginConfig = config.plugins.entries['hummem'].config;
      if (pluginConfig) {
...
export async function installOpenClawIntegration(): Promise<number> {
  console.log('\nInstalling hummem for OpenClaw...\n');
