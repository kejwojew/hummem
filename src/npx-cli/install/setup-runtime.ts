...
export function platformBunRemediation(): string {
  return IS_WINDOWS
    ? 'Install Bun manually: `winget install Oven-sh.Bun` (or `powershell -c "irm bun.sh/install.ps1 | iex"`), then re-run `npx hummem install`.'
    : 'Install Bun manually: `curl -fsSL https://bun.sh/install | bash` (or `brew install oven-sh/bun/bun`), then re-run `npx hummem install`.';
}

export function platformUvRemediation(): string {
  return IS_WINDOWS
    ? 'Install uv manually: `winget install astral-sh.uv` (or `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"`), then re-run `npx hummem install`.'
    : 'Install uv manually: `curl -LsSf https://astral.sh/uv/install.sh | sh` (or `brew install uv`), then re-run `npx hummem install`.';
}
...
function userHasOptedOutOfVectorSearch(): boolean {
  ...
  try {
    ...
    console.warn(`hummem: could not read ${USER_SETTINGS_PATH} while checking vector-search opt-out:`, err);
    return false;
  }
...
export function getBunVersion(): string | null {
  ...
  try {
    ...
    const err = error instanceof Error ? error : new Error(String(error));
    console.warn('hummem: bun --version probe failed:', err);
    return null;
  }
}
...
export function getUvVersion(): string | null {
  ...
  try {
    ...
    const err = error instanceof Error ? error : new Error(String(error));
    console.warn('hummem: uv --version probe failed:', err);
    return null;
  }
}
