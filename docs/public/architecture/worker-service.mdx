...
Bun is required to run the worker service. If Bun is not installed, `npx hummem install` (and `npx hummem repair`) installs it globally during setup, with a visible clack spinner:
