...
(Runtime setup is handled out-of-band by `npx hummem install` / `npx hummem repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
...
// Removes <private>...</private> and <hummem-context>...</hummem-context>
