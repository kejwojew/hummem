...
|------|--------|--------------|-------------------|
| **Stable** | `main` | Everyone. This is the `npx hummem` install. | **Yes** - the only published line |
...
- **Just want memory that works?** Use stable. `npx hummem@latest` is always `main`.
...
- An npm publish is what makes `npx hummem@<version>` resolve.
