...
Pushing the tag is what publishes: `.github/workflows/npm-publish.yml` fires on
any `v*` tag and runs build → clean-room smoke → `npm publish` with the
repository's `NPM_TOKEN` secret. Do not publish by hand — a local `npm publish`
(or `np`, or `npm run release:*`) races the workflow for the same version
number, and whichever loses fails on an already-published version.

```bash
gh run list --workflow npm-publish.yml --limit 1
npm view hummem@NEW_VERSION version
```
