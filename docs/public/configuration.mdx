...
`npx hummem install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.claude-mem/settings.json`.
...
├── .install-version        # Version marker written by `npx hummem install`/`repair`
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx hummem install` / `npx hummem repair` outside the session lifecycle.
