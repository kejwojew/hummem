...
> Always install via `npx hummem install` or the `/plugin` commands above.

## System Requirements

- **Node.js**: 20.0.0 or higher
- **Bun** ≥ 1.0 (auto-installed by `npx hummem install` if missing)
...
Dependencies are installed automatically by `npx hummem install` and `npx hummem repair`. Heavy lifting (Bun + uv install, `bun install` inside the plugin cache) happens behind a visible installer spinner. The Setup hook only performs a sub-100ms `version-check.js` read of the `.install-version` marker — on mismatch it prints `run: npx hummem repair` to stderr and exits 0, so it never blocks a session. Works cross-platform on Windows, macOS, and Linux.
...
Upgrades are automatic when updating via the plugin marketplace. After an external upgrade (for example `claude plugin update`), the Setup hook detects a version-marker mismatch and asks you to run `npx hummem repair`, which installs any missing runtime dependencies and refreshes the marker.

See [CHANGELOG](https://github.com/kejwojew/hummem/blob/main/CHANGELOG.md) for complete version history.
