...
prompt text to the sync hub under your account. Don't enable it if
...
1. A **sync token and user id** issued by the sync server you use.
2. A **sync hub URL** in `CLAUDE_MEM_CLOUD_SYNC_HUB_URL`. The hub is a
   ...
   deploy runbook); use the hub URL your sync server documentation
   provides for it.
...
| Key | Default | Meaning |
|-----|---------|---------|
| `CLAUDE_MEM_CLOUD_SYNC_TOKEN` | `''` | Your sync token (issued by your sync server). Sent as `Authorization: Bearer`. |
| `CLAUDE_MEM_CLOUD_SYNC_USER_ID` | `''` | Your the sync server user id (from **the sync server → Connect**). All your devices share it — it names your hub log. |
