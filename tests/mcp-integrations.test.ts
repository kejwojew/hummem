...
function writeMcpJsonConfig(
  ...
  existingConfig[serversKeyName]['hummem'] = buildMcpServerEntry(mcpServerPath);
...
describe('MCP Integrations', () => {
  ...
  describe('writeMcpJsonConfig', () => {
    ...
    it('writes valid JSON with hummem entry', () => {
      ...
      expect(config.mcpServers['hummem'].command).toBe(process.execPath);
      expect(config.mcpServers['hummem'].args).toEqual(['/path/to/mcp.cjs']);
    });
    ...
    it('uses custom serversKeyName when provided', () => {
      ...
      expect(config.servers['hummem']).toBeDefined();
      expect(config.mcpServers).toBeUndefined();
    });

    it('preserves existing servers when adding hummem', () => {
      const configPath = join(tempDir, 'mcp.json');
  ...
  describe('idempotency', () => {
    it('running install twice does not create duplicate entries', () => {
      ...
      const hummemEntries = serverKeys.filter((k) => k === 'hummem');
      expect(hummemEntries).toHaveLength(1);
    });
    ...
    it('updates the server path on re-install', () => {
      ...
      const config = JSON.parse(readFileSync(configPath, 'utf-8'));
      expect(config.mcpServers['hummem'].args).toEqual(['/new/path/mcp.cjs']);
    });
  });
