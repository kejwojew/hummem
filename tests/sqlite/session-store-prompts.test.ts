...
describe('SessionStore prompts', () => {
  ...
  describe('saveUserPrompt', () => {
    ...
    it('stores a tag-stripped, bounded prompt_text ending in an ellipsis', () => {
      ...
      const oversized = `<hummem-context>ignored</hummem-context>${'A'.repeat(MAX_STORED_PROMPT_CHARS + 250)}`;
      ...
      expect(stored.prompt_text.startsWith('<hummem-context>')).toBe(false);
