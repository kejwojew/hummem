...
describe('worker-json-status', () => {
  ...
  describe('formatDependencyHealthHint', () => {
    it('returns a short dependency degradation hint when health reports degraded dependencies', () => {
      ...
      expect(hint).toBe('  Dependencies: degraded (Claude CLI setup required, uvx unavailable for vector search, Chroma unavailable for vector search). Run npx hummem doctor or open Settings for remediation.');
    });
