...
describe('Cursor MCP Configuration', () => {
  ...
  describe('configureCursorMcp', () => {
    ...
    it('adds hummem server with correct structure', () => {
      configureCursorMcp(mcpJsonPath, mcpServerPath);

      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));

      expect(config.mcpServers).toBeDefined();
      expect(config.mcpServers['hummem']).toBeDefined();
      expect(config.mcpServers['hummem'].command).toBe('node');
      expect(config.mcpServers['hummem'].args).toEqual([mcpServerPath]);
    });

    it('preserves existing MCP servers when adding hummem', () => {
    ...
    it('updates existing hummem server path', () => {
      ...
      expect(config.mcpServers['hummem'].args).toEqual([newPath]);
    });
  ...
  describe('path handling', () => {
    it('handles absolute path with spaces', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['hummem'].args).toEqual([pathWithSpaces]);
    });
    ...
    it('handles Windows-style path', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['hummem'].args).toEqual([windowsPath]);
    });
    ...
    it('handles path with special characters', () => {
      ...
      const config: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(config.mcpServers['hummem'].args).toEqual([specialPath]);

      const reread: CursorMcpConfig = JSON.parse(readFileSync(mcpJsonPath, 'utf-8'));
      expect(reread.mcpServers['hummem'].args![0]).toBe(specialPath);
    });
  });
});
